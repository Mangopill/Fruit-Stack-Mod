package com.fruitstack.fruitstack.common.core;
...
public class NewRecipeManager extends SimpleJsonResourceReloadListener {
   ...
   /** @deprecated Forge: use {@linkplain NewRecipeManager#NewRecipeManager(net.minecraftforge.common.crafting.conditions.ICondition.IContext) constructor with context}. */
   @Deprecated
   public NewRecipeManager() {
      this(net.minecraftforge.common.crafting.conditions.ICondition.IContext.EMPTY);
   }

   public NewRecipeManager(net.minecraftforge.common.crafting.conditions.ICondition.IContext context) {
      super(GSON, "recipes");
   ...
   public <C extends Container, T extends Recipe<C>> Optional<T> getRecipeFor(RecipeType<T> p_44016_, C p_44017_, Level p_44018_) {
      return this.fruitstack_invokeGetRecipeMap(p_44016_).values().stream().flatMap((p_44064_) -> {
   ...
   public <C extends Container, T extends Recipe<C>> List<T> getAllRecipesFor(RecipeType<T> p_44014_) {
      return this.fruitstack_invokeGetRecipeMap(p_44014_).values().stream().map((p_44053_) -> {
         return (T)p_44053_;
      }).collect(Collectors.toList());
   }

   public <C extends Container, T extends Recipe<C>> List<T> getRecipesFor(RecipeType<T> p_44057_, C p_44058_, Level p_44059_) {
      return this.fruitstack_invokeGetRecipeMap(p_44057_).values().stream().flatMap((p_44023_) -> {
   ...
   public <C extends Container, T extends Recipe<C>> Map<ResourceLocation, Recipe<C>> fruitstack_invokeGetRecipeMap(RecipeType<T> type) {
      return (Map<ResourceLocation, Recipe<C>>)(Map<ResourceLocation, T>)this.recipes.getOrDefault(type, Collections.emptyMap());
   }
